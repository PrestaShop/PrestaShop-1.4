...
class OrderOpcControllerCore extends FrontController
{
	...
	public function __construct()
	{
		...
		$this->isLogged = (bool)(intval($this->cookie->id_customer) AND Customer::customerIdExistsStatic(intval($this->cookie->id_customer)));
	}
					...
					switch (Tools::getValue('method'))
					{
						case 'updateMessage':
							if (Tools::isSubmit('message'))
							{
								...
								$oldMessage = Message::getMessageByCartId(intval($this->cart->id));
								if ($txtMessage)
								{
						...
						case 'updateCarrier':
							...
							{
								$this->cart->recyclable = intval(Tools::getValue('recyclable'));
								$this->cart->gift = intval(Tools::getValue('gift'));
								...
								$address = new Address(intval($this->cart->id_address_delivery)); // dynamise for id country
								...
								if (Validate::isInt(Tools::getValue('id_carrier')) AND sizeof(Carrier::checkCarrierZone(intval(Tools::getValue('id_carrier')), intval($id_zone))))
									$this->cart->id_carrier = intval(Tools::getValue('id_carrier'));
								elseif (!$this->cart->isVirtualCart() AND intval(Tools::getValue('id_carrier')) != 0)
						...
						case 'updateTOSStatus':
							...
							{
								$this->cookie->checkedTOS = intval(Tools::getValue('checked'));
								die(true);
							}
							break;
						case 'getCarrierList':
							...
							if ($this->cookie->id_customer)
							{
								...
								$groups = array(1);
							if (!Address::isCountryActiveById(intval($this->cart->id_address_delivery)))
						...
						case 'getPaymentModule':
							...
							if ($orderTotalDefaultCurrency < $minimalPurchase)
								die('<p class="warning">'.Tools::displayError('A minimum purchase total of').' '.Tools::displayPrice($minimalPurchase, Currency::getCurrency(intval($this->cart->id_currency))).
	...
	public function process()
	{
		...
		$currency = new Currency(intval($this->cookie->id_currency));
		...
		if (intval($this->cookie->id_customer) AND Customer::customerIdExistsStatic(intval($this->cookie->id_customer)))
		{
			// ADDRESS
