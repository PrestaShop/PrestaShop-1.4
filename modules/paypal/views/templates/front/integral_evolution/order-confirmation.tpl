...
<p>{l s='Total of the transaction (taxes incl.) :'} <span class="bold">{$order.total_paid}{$currency->sign} ({$order.currency})</span></p>
<p>{l s='Your order ID is :'} <span class="bold">{$order.id_order}</span></p>
<p>{l s='Your invoice number is :'} <span class="bold">{$order.id_invoice}</span></p>
