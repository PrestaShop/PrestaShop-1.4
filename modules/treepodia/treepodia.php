...
{	
	...
	function install()
	{
		...
		$logo = Configuration::get('TREEPODIA_PLAY_LOGO') ? Configuration::get('TREEPODIA_PLAY_LOGO') : '01_1.jpg';
		$position = Configuration::get('TREEPODIA_POSITION') ?  Configuration::get('TREEPODIA_POSITION') : 0;
...
XML;
	...
	{
		if (isset($_GET['account-code']) AND isset($_GET['token']) AND $_SERVER['REMOTE_ADDR'] == '80.12.80.104')
		{
			$accountCode = Tools::getValue('account-code');
			$token = Tools::getValue('token');
			$realToken = Configuration::get('TREEPODIA_TOKEN');
			if ($this->_isAccountCode($accountCode) AND Validate::isPasswd($token) AND $token == $realToken)
			{
				Configuration::updateValue('TREEPODIA_ACCOUNT_CODE', Tools::htmlentitiesUTF8($accountCode));
				echo 'OK';
			}
		}
	}
