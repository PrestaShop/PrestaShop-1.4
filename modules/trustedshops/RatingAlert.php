...
class RatingAlert
{
	...
	{
		Db::getInstance()->AutoExecute(_DB_PREFIX_.self::TABLE_NAME, array('id_order' => intval($id_order)), 'INSERT');
	}
	...
	private static function _getAlertsInformations($nb_days = 10)
	{
		...
		WHERE DATE_ADD(o.`date_add`, INTERVAL '.intval($nb_days).' DAY) <= NOW()');
	}
	...
	public static function executeCronTask()
	{
		...
		$to_remove = array();
		$alerts_infos = RatingAlert::_getAlertsInformations(intval(Configuration::get('TS_SEND_SEPERATE_MAIL_DELAY')));
		...
		foreach ($alerts_infos AS $infos)
		{
			...
			if ($result)
				$to_remove[] = intval($infos['id_alert']);
		}
