...
class StatsSales extends ModuleGraph
{
	...
	public function hookAdminStatsModules($params)
	{
		...
		$this->_html = '
		<fieldset class="width3"><legend><img src="../modules/'.$this->name.'/logo.gif" alt="" /> '.$this->displayName.'</legend>
		...
		foreach (Country::getCountries($cookie->id_lang, false, false, false) as $country)
			$this->_html .= '<option value="'.(int)$country['id_country'].'"'.(($country['id_country'] == Tools::getValue('id_country')) ? ' selected="selected"' : '').'>'.$country['name'].'</option>';
		$this->_html .= '</select>
				<input type="submit" name="submitCountry" value="'.$this->l('Filter').'" class="button" />
			</form>
			<p><center><img src="../img/admin/down.gif" alt="" />
			...
			<p class="space"><img src="../img/admin/down.gif" alt="" />
				'.$this->l('You can see the order state distribution below.').'
			</p><br />
			<center>'.($totals['orderCount'] ? ModuleGraph::engine(array('type' => 'pie', 'option' => '3-'.(int)Tools::getValue('id_country'))) : $this->l('No orders for this period.')).'</center>
			<p><a href="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'&export=3"><img src="../img/admin/asterisk.gif" alt="" />'.$this->l('CSV Export').'</a></p>
		</fieldset>
		<br class="clear" />
		<fieldset class="width3">
			<legend><img src="../img/admin/comment.gif" alt="" /> '.$this->l('Guide').'</legend>
			<h2>'.$this->l('Various order statuses').'</h2>
			<p>'.$this->l('In your back-office, you can find the following order statuses : Awaiting check payment, Payment accepted, Preparation in progress, Shipping, Delivered, Cancelled, Refund, Payment error, Out of stock, and Awaiting bank wire payment.').'<br />
			'.$this->l('Statuses cannot be removed from the back-office, but you have the option to add more.').'</p>
		</fieldset>';
	...
	private function getTotals()
	{
		...
		SELECT COUNT(o.`id_order`) orderCount, SUM(o.`total_paid_real` / o.conversion_rate) orderSum
		...
		SELECT SUM(od.product_quantity) products
		FROM `'._DB_PREFIX_.'orders` o
	...
	protected function getData($layers)
	{
		...
		$this->_query = '
			SELECT o.`invoice_date`, (o.`total_paid_real` / o.conversion_rate) total_paid_real, SUM(od.product_quantity) product_quantity
			FROM `'._DB_PREFIX_.'orders` o
			LEFT JOIN `'._DB_PREFIX_.'order_detail` od ON od.`id_order` = o.`id_order`
	...
	private function getStatesData()
	{
		...
		SELECT osl.`name`, COUNT(oh.`id_order`) total
