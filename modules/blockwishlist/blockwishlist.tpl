	...
	<div class="block_content">
		...
		{if $products}
			...
			{foreach from=$products item=product name=i}
				<dt class="clear {if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					...
					<a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite)}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|truncate:30:'...':TRUE|escape:'htmlall':'UTF-8'}</a>
		...
		{if $wishlists}
			...
			{foreach from=$wishlists item=wishlist name=i}
				<option value="{$wishlist.id_wishlist}"{if $id_wishlist eq $wishlist.id_wishlist or ($id_wishlist == false and $smarty.foreach.i.first)} selected="selected"{/if}>{$wishlist.name|truncate:22:'...':TRUE|escape:'htmlall':'UTF-8'}</option>
			{/foreach}
			</select>
		{/if}
