	...
	<div class="block_content">
		...
		{foreach from=$productsViewedObj item=viewedProduct name=myLoop}
			<li class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{else}item{/if}">
				<a href="{$link->getProductLink($viewedProduct)}" title="{l s='More about' mod='blockviewed'} {$viewedProduct->name|escape:htmlall:'UTF-8'}"><img src="{$img_prod_dir}{$viewedProduct->cover}-medium.jpg" alt="{$viewedProduct->legend|escape:htmlall:'UTF-8'}" /></a>
