...
<div style="float:right;width:400px;margin-bottom:10px">
  <fieldset>
    <h3>Opening your CloudCache.com Account</h3>
...
{*    <h3>{l s='Module purpose:' mod='cloudcache'}</h3>
  ...
  <div id="menuTab3Sheet" class="tabItem">
    <form action="{$serverRequestUri}&id_tab=3" method="post">
	...
	<b style="color: green;">{l s='Success! That\'s all you have to do!' mod='cloudcache'} {$connectionTestResult['newZone']} {l s='is now accelerated by cloudcache.com' mod='cloudcache'}</b>
	{else}
	...
	<div class="bandwidth-value">{$prepaidBandwith} Tb left</div>
      </div>
      {/if*}
    </div>    
	...
	{if 0 && !empty($companyId)}<tr>
	  ...
	  <td><input type="text" name="cloudcache_api_company_id" value="{if isset($companyId)}{$companyId}{/if}" /></td>
      </tr> {/if}
	<tr>
	  <td class="cloudcache_column">{l s='API User' mod='cloudcache'}</td>
	  <td><input type="text" name="cloudcache_api_user" id="cloudcache_api_user" value="{if isset($apiUser)}{$apiUser}{/if}" /></td>
	</tr>
	<tr>
	  <td class="cloudcache_column">{l s='API Key' mod='cloudcache'}</td>
	  <td><input type="{if isset($apiKey) && !empty($apiKey)}password{else}text{/if}" id="cloudcache_api_key" name="cloudcache_api_key" value="{if isset($apiKey)}{$apiKey}{/if}"/></td>
      </tr>
      </table>
      ...
      <center><img src="../img/admin/exchangesrate.gif" alt="" /> <input type="submit" onClick="$('#loading').show();" id="cloudcache_test_connection" class="button cloudcache_button" name="SubmitCloudcacheTestConnection" value="{l s='Click here to Test Connection' mod='cloudcache'}" style="margin-top: 0;" /><div id="loading" >{l s='Loading' mod='cloudcache'} <img src="{$base_dir}modules/cloudcache/loading.gif" style="height: 25px;" /></div></center>
    </form>
  </div>
  {if isset($couponUrl) && isset($apiActive) && $apiActive == 1}
  <div id="menuTab4Sheet" class="tabItem">
	...
	<table style="display: inline-block;" class="spaced-table2">
	  ...
	  <tr align="left" valign="top">
	    <td>{$zone.name}</td>
	    <td>{if $zone['file_type'] eq null or $zone['file_type'] eq 'none' or $zone['file_type'] eq '0'}{l s='N/A' mod='cloudcache'}{else}{$zone['file_type']|upper}{/if}</td>
	    <td><div class="jexcerpt-short">{$zone['origin']|substr:0:10}{if $zone['origin']|strlen > 10}...{/if}</div>{if $zone['origin']|strlen > 10}<div class="jexcerpt-long">{$zone['origin']}</div>{/if}</td>
	    <td><span class="jexcerpt-short">{$zone['cdn_url']|substr:0:10}{if $zone['cdn_url']|strlen > 10}...{/if}</span>{if $zone['cdn_url']|strlen > 10}<span class="jexcerpt-long">{$zone['cdn_url']}</span>{/if}</td>
	    {*	    <td>{if isset($zone.label)}{$zone.label}{/if}</td> *}
	    <td>{if isset($zone['compress']) && $zone['compress'] == 1}{l s='YES' mod='cloudcache'}{else}{l s='NO' mod='cloudcache'}{/if}</td>
    <td>{if $zone.bw_last_month != -1}{$zone.bw_last_month}{else}{l s='N/A' mod='cloudcache'}{/if}</td>
	    <td>{if $zone.bw_last_week != -1}{$zone.bw_last_week}{else}{l s='N/A' mod='cloudcache'}{/if}</td>
	    <td>{if $zone.bw_yesterday != -1}{$zone.bw_yesterday}{else}{l s='N/A' mod='cloudcache'}{/if}</td>

	    <td>
	      <form method="post" action="{$serverRequestUri}&id_tab=4">
		<input type="hidden" value="{$id_zone}" name="id_zone"/>
    ...
    {/if}
    <form method="post" action="{$serverRequestUri}&id_tab=4" class="MT20">
      <div class="R">
    ...
    <div id="cloudcache_edit_zone_form" class="cloudcache-dialogbox">
      <form method="post" action="{$serverRequestUri}&id_tab=4">
	<input type="hidden" name="id_zone" id="id_zone" value="{$edit_zone_info.id_zone}"/>
	<table border="0" cellspacing="5" class="bold-td">
	  <tr>
	    <td>{l s='ID Zone' mod='cloudcache'}</td>
	    <td>{$edit_zone_info['id_zone']}</td>
	  </tr>
	  <tr>
	    <td>{l s='Pull Zone Name' mod='cloudcache'}</td>
	    <td><input type="hidden" name="name" id="name" size="20" maxlength="30" value="{$edit_zone_info['name']}"/>{$edit_zone_info['name']}</td>
	  </tr>
	  <tr>
	    <td>{l s='Origin Server Url' mod='cloudcache'}</td>
	    <td><input type="hidden" name="origin" id="origin" size="20" maxlength="30" value="{$edit_zone_info['origin']}"/>{$edit_zone_info['origin']}</td>
	  </tr>
	  <tr>
	    <td>{l s='Custom CDN Domain' mod='cloudcache'}</td>
	    <td><input type="text" name="vanity_domain" id="vanity_domain" size="20" maxlength="30" value="{$edit_zone_info['cdn_url']}"/></td>
	  </tr>
	  <tr>
	    <td>{l s='Label' mod='cloudcache'}</td>
	    <td><input type="text" name="label" id="label" size="20" maxlength="30" value="{$edit_zone_info['label']}"/></td>
	  </tr>
	  <tr>
...
{*	  <tr>
		...
		{foreach from=$allAvailableZones key=type item=name}
		<option value="{$type}" {if $edit_zone_info['type'] == $type}selected="selected"{/if}>{$name}</option>
		{/foreach}
	      </select>
	    </td>
	  </tr>
*}
	...
	<table border="0" cellspacing="5" class="bold-td">
	  ...
	  <tr>
	    ...
	    <td><input type="text" name="origin" id="origin" size="20" maxlength="30" value="{$defaultOriginServerURL}"/></td>
	  </tr>
	  <tr>
	    <td>{l s='Custom CDN Domain' mod='cloudcache'}</td>
