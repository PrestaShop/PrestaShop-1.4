			<fieldset>
				...
				<table class="table" width="100%" >
					...
					<tr class='bold'>
						<th>{l s='#' mod='dejala'}</td>
						<th>{l s='Creation' mod='dejala'}</td>
						<th>{l s='Order ID' mod='dejala'}</td>
						<th>{l s='Tracking_number' mod='dejala'}</td>
						<th>{l s='Label' mod='dejala'}</td>
						<th>{l s='Price' mod='dejala'}</td>
						<th>{l s='Status' mod='dejala'}</td>
						<th>{l s='Shipping' mod='dejala'}</td>
						<th>{l s='Description' mod='dejala'}</td>
					</tr>
					...
					{foreach from=$deliveries item=delivery name=deliveryLoop}
						...
						{/if}	
							...
							<td>{$product.description}{l s='In ' mod='dejala'}{$delivery.timelimit}{l s='h' mod='dejala'} - {l s='from' mod='dejala'} {$delivery.shipping_start} {l s='to' mod='dejala'} {$delivery.shipping_stop}</td>
						</tr>
					{/foreach}
				</table>
				<br/>
