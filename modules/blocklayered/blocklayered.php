...
class BlockLayered extends Module
{
	...
	public function hookAfterSaveFeature($params)
	{
		...
		foreach (Language::getLanguages(false) as $language)
		{
			...
			\''.pSQL(Tools::htmlentitiesUTF8(Tools::getValue('meta_title_'.$id_lang))).'\')');
		}
	}
		...
		foreach ($languages as $language)
			$return .= '
					<div id="url_name_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $default_form_language ? 'block' : 'none').'; float: left;">
						<input size="33" type="text" name="url_name_'.$language['id_lang'].'" value="'.Tools::htmlentitiesUTF8(@$langValue[$language['id_lang']]['url_name']).'" />
		...
		foreach ($languages as $language)
			$return .= '
						<div id="meta_title_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $default_form_language ? 'block' : 'none').'; float: left;">
							<input size="33" type="text" name="meta_title_'.$language['id_lang'].'" value="'.Tools::htmlentitiesUTF8(@$langValue[$language['id_lang']]['meta_title']).'" />
