...
$instantSearch = Tools::getValue('instantSearch') AND $query = urldecode(Tools::getValue('q')) AND !is_array($query);
$ajaxSearch = Tools::getValue('ajaxSearch') AND $query = urldecode(Tools::getValue('q')) AND !is_array($query);
...
if ($instantSearch && !is_array($query))
{
...
elseif ($tag = Tools::getValue('tag') AND !is_array($tag))
{
	...
	$smarty->assign(array(
	'search_tag' => $tag,
	'products' => Search::searchTag(intval($cookie->id_lang), $tag, false, $p, $n, $orderBy, $orderWay),
