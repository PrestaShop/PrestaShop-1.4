...
if ($postProducts)
{
	...
	if (sizeof($ids) > 0)
	{
		...
		foreach ($ids AS $id)
		{			
			$curProduct = new Product(intval($id), true, intval($cookie->id_lang));
			if (!Validate::isLoadedObject($curProduct))
				continue;
				
			foreach ($curProduct->getFrontFeatures($cookie->id_lang) AS $feature)
		...
		{	
			...
			$hasProduct = true;
			$ordered_features = Feature::getFeaturesForComparison($ids, $cookie->id_lang);
			$smarty->assign(array('ordered_features' => $ordered_features,
...
$smarty->display(_PS_THEME_DIR_.'products-comparison.tpl');
