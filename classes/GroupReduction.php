...
class GroupReductionCore extends ObjectModel
{
	...
	{
		return (parent::update($nullValues) AND Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'product_group_reduction_cache` WHERE `id_group` = '.intval($this->id_group)) AND $this->_setCache());
	}
	...
	private function _setCache()
	{
		$resource = Db::getInstance()->ExecuteS('
			...
			WHERE cp.`id_category` = '.intval($this->id_category)
		, false);
		$query = 'INSERT INTO `'._DB_PREFIX_.'product_group_reduction_cache` (`id_product`, `id_group`, `reduction`) VALUES ';
		while ($row = Db::getInstance()->nextRow($resource))
			$query .= '('.intval($row['id_product']).', '.intval($this->id_group).', '.floatval($this->reduction).'), ';
	...
	static public function getGroupReductions($id_group, $id_lang)
	{
		return Db::getInstance()->ExecuteS('
			...
			LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (cl.`id_category` = gr.`id_category` AND cl.`id_lang` = '.intval($id_lang).')
