...
class AdminGenerator extends AdminTab
{
	...
	public function display()
	{
		...
		<p><b>'.$this->l('Warning:').' </b>'.$this->l('this tool can be ONLY used if you are hosted by an').' <b><a href="http://www.apache.org">'.$this->l('Apache web server').'</a></b>'.$this->l('.').' '.$this->l('Please ask to your webhost.').'</p>';
		echo '<p>'.$this->l('This tool will automaticaly generate a ".htaccess" file, that will grant you the possibility to do URL rewriting and to catch 404 errors.').'</p>';
		echo '<p>'.$this->l('If you did not have enabled the url-rewriting when generating the ".htaccess" file, such feature won\'t be available').'.</p>';
		if ($this->_checkConfiguration($this->_htFile))
				echo '<p style="font-weight:bold;">'.$this->l('Generate your ".htaccess" file by clicking on the following button:').'<br /><br />
				<input type="submit" value="'.$this->l('Generate .htaccess file').'" name="submitHtaccess" class="button" /></p>';
		else
		{
			echo '<p style="color:red; font-weight:bold;">'.$this->l('Before being able to use this tool, you need to:').'</p>';
			echo '<p>'.$this->l('- create a').' <b>'. $this->l('.htaccess').'</b> '.$this->l('blank file in dir:').' <b>'.__PS_BASE_URI__.'</b>';
			echo '<br />'.$this->l('- give it write permissions (chmod 777 on Unix system)').'</p>';
		}
		echo '</p></fieldset>
		</form>';
		...
		echo '<p>'.$this->l('eg: http://www.yoursite.com/robots.txt').'.</p>';
		echo '<p>'.$this->l('This tool will automaticaly generate a "robots.txt" file, that will grant you the possibility to denie access to search engines for somes pages.').'</p>';
		if ($this->_checkConfiguration($this->_rbFile))
				echo '<p style="font-weight:bold;">'.$this->l('Generate your "robots.txt" file by clicking on the following button:').'<br /><br />
				<input type="submit" value="'.$this->l('Generate robots.txt file').'" name="submitRobots" class="button" /></p>';
		else
		{
			echo '<p style="color:red; font-weight:bold;">'.$this->l('Before being able to use this tool, you need to:').'</p>';
			echo '<p>'.$this->l('- create a').' <b>'. $this->l('robots.txt').'</b> '.$this->l('blank file in dir:').' <b>'.__PS_BASE_URI__.'</b>';
			echo '<br />'.$this->l('- give it write permissions (chmod 777 on Unix system)').'</p>';
		}
		echo '</p></fieldset>
		</form>';
	...
	public function _getRobotsContent()
	{
		...
		'order-slip.php', 'order-detail.php', 'order-follow.php', 'order-return.php', 'order-confirmation.php', 'pagination.php',
